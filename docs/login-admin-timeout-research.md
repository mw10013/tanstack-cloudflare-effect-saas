...
## Follow-up: is this the old dynamic-import bug?

Short answer: likely **no**.

### Why

- workers-sdk issue `#12924` (dynamic import hang) is closed.
- fix PR `#13056` is merged.
- workers-sdk changelog says fix shipped in `@cloudflare/vitest-pool-workers` `0.13.5`.
- this repo is on `@cloudflare/vitest-pool-workers` `0.14.3` (`package.json:95`).
- installed package includes the `onModuleRunner` transport patch (`node_modules/.pnpm/.../dist/worker/index.mjs:803`).

So the known pre-`0.13.5` failure mode is already patched in our dependency set.

### What this implies

This timeout looks more like longstanding integration flakiness in this stack (Workers Vitest + app SSR/server-fn path) than a simple "upgrade to get dynamic-import fix" regression story.

## Practical decision

Given:

- auth integration tests pass when this smoke test is not in the path,
- this one smoke test introduces nondeterministic infra/runtime coupling,
- and it is not proving core auth behavior beyond what other tests already cover,

removing `renders /login` from integration smoke is a pragmatic option.

If still desired, keep that coverage in browser e2e instead, where page-render checks are a more natural fit.
